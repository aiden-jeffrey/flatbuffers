...
func _init(p_initial_size: int = 1024, p_force_defaults: bool = false) -> void:
  self.buffer = FB__ByteBuffer.make_blank(p_initial_size)
  self.head = self.buffer.capacity()
  self.force_defaults = p_force_defaults

# ensure there is head and pads HEAD to elem_size
func prep(p_elem_size: int, p_additional_bytes: int) -> bool:
  if p_elem_size > self.min_align:
    self.min_align = p_elem_size

  # NB: I have no idea how this works
  var align_size = (~(self.buffer.capacity() - self.head + p_additional_bytes) + 1) & (p_elem_size - 1)

  # TODO: i think this allocated too much head
  while (self.head < align_size + p_elem_size + p_additional_bytes):
...
func pack_object_array(p_obj_arr: Array) -> Array[int]:
  ...
  for i in range(p_obj_arr.size()):
    ...
    if val != null:
      ...
      printerr("bad object in obj array")
...
func gen_trimmed_buffer() -> PackedByteArray:
  if !self.finished:
    printerr("need to finish buffer first")

  return self.buffer.bytes.slice(
    self.buffer.position, self.buffer.position + self.offset())
